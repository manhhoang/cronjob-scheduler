...
func newBufLogger(sw *syncWriter) utility.Logger {
	return utility.PrintfLogger(log.New(sw, "", log.LstdFlags))
}

func TestFuncPanicRecovery(t *testing.T) {
	var buf syncWriter
	cron := New(WithParser(NewParser(
...
func TestNonLocalTimezone(t *testing.T) {
	...
	cron := New(WithLocation(loc), WithParser(NewParser(
		SecondOptional | Minute | Hour | Dom | Month | Dow | Descriptor,
	)))
	cron.AddFunc(spec, func() { wg.Done() })
	cron.Start()
...
func newWithSeconds() *Cron {
	return New(WithParser(NewParser(
		SecondOptional | Minute | Hour | Dom | Month | Dow | Descriptor,
	)), WithChain())
}
